...
class pdfobj:
    ...
    def __init__(self, keynum, data=''):
        ...
        self.hiddenTags = [] # tags containing non-normalized data
        self.children = [] # if this has a script tag, parse children of it
        self.parent = []
        self.Kids = [] # this is for the keyword Kids, parse the chidren of it
    ...
    def parseTag(self, tag, stream):
        ...
        for source, tagtype, tagdata in uncleaned_tags:
            ...
            if newtagtype != tagtype:
                self.hiddenTags.append(tagtype)
                tagtype = newtagtype
...
class pdf:
    ...
    def __init__(self, indata, infile, password= ''):
        ...
        self.pages = []
        self.list_obj = []
        self.repeated_objs = [] # 记录重复出现过的obj
        self.rubbish_objs = []
        self.jsObjects = []
        self.encryptKey = ''
    ...
    def parse(self):
        """
        解析之后，所有obj都将统一以"数字+空格+数字+obj|R"形式保存
        """
        '''
        # pdf规定只有在关键字xref之前的obj才有效；如果没有xref，就是只有在trailer之前的obj才有效
        endPos = self.indata.rfind("xref")
        print 'endPos', str(endPos)
        if endPos == -1:
            endPos = self.indata.rfind("trailer")
       
        """
        objs = re.findall('\n?(\d+\s+\d+\s+obj).*?', self.indata, re.MULTILINE | re.DOTALL) 
        objDict = {} # 用来记录每一个obj_no : pos

        for obj in objs:
            result = re.search('^(\d+)\s+(\d+)\s+obj', obj)
            obj_no = result.group(1)
            obj_gen = result.group(2)
            if obj_no not in objDict.keys():
                pos = self.indata.find(obj)
                objDict[obj_no] = [pos, obj, obj_gen]
            else:
                start_pos = objDict[obj_no][0] + 1
                new_pos = self.indata[start_pos:].find(obj) + start_pos
                # record repeated obj
                self.repeated_objs.append([obj_no, start_pos - 1, obj])
                
                if new_pos > endPos: # record rubbish obj
                    self.rubbish_objs.append([obj_no, new_pos, obj])
                else:
                    objDict[obj_no] = [new_pos, obj, obj_gen]
        """
        ...
        for obj in objs:
            ...
            obj_no = result.group(1)
            obj_gen = result.group(2)
            pos = self.indata[cur_pos:].find(obj) + cur_pos
            if obj_no not in objDict.keys():
                objDict[obj_no] = [pos, obj, obj_gen]
            else:
                # record repeated obj
                self.repeated_objs.append(objDict[obj_no])
                if cur_pos > endPos: # record rubbish obj
                    self.rubbish_objs.append(objDict[obj_no])
                else:
                    objDict[obj_no] = [cur_pos, obj, obj_gen]     
            cur_pos = pos + 1
        ...
        for key in objDict.keys():
            ...
            obj = objDict[key][1]
            gen = objDict[key][2]
            objList.append([obj_no, pos, obj, gen])
        objList = sorted(objList, key=lambda pos:pos[1])

        objs = []
        num = len(objList)
        for i in range(num):
            obj_no, pos, obj, gen = objList[i]
            nextPos = 0
            if i < num-1:
                nextPos = objList[i+1][1]
                
                # 下面判断如果有repeated obj情况，划分时需要重新划定nextPos
                m = re.search(r'.*?(\d+\s+\d+\s+obj).*?', self.indata[pos+len(obj):nextPos])
                if m and m.groups():
                    nextobj = m.group(0)
                    nextPos = self.indata[pos+len(obj):nextPos].find(nextobj) + pos + len(obj)
                
            else:
                nextPos = endPos
                print pos
                print nextPos
                print self.indata[pos:nextPos]
            data = self.indata[pos+len(obj):nextPos]
            objs.append((obj_no, gen, data))
        '''
        objs = re.findall('\n?(\d+)\s+(\d+)\s+obj(.*?)(endobj|objend)', self.indata, re.MULTILINE | re.DOTALL)
        if objs:
            for obj in objs:
                # fill all objects
                key = obj[0] + ' ' + obj[1]
            ...
            for key in self.list_obj: #sorted(self.objects.keys()):
                            ...
                            for filter in filters:
                                if filter == 'FlateDecode' or filter == 'Fl': 
                                    try:
                                        ...
                                        ### logging.debug('failed to decompress object %s (inlen %d)' % (key, len(self.objects[key].tagstream)))
                                        # raise # lyz
                ...
                # To get some obj's type (lyz 2017.12.4)
                for kstate, k, kval in self.objects[key].tags:
                    if k in ['OpenAction', 'Pages', 'Outlines', 'Root']: # can added others later
                        result = re.search('^(\d+)\s+(\d+)\s+R', kval)
                        if result:
                            obj_key = result.group(1) + ' ' + result.group(2)
                            if self.objects.has_key(obj_key):
                                self.objects[obj_key].type = k
                
        else:
            print 'Fatal error: pdf has no objects in ' + self.infile
            raise TypeError('Fatal error: pdf has no objects in ' + self.infile)
    ...
    def computeEncryptKey(self, encryptObject, password, fileId):
        if encryptObject['R'] <= 4:
            ...
            h.update(fileId)
            if encryptObject['R'] == 4 and encryptObject.has_key('encryptMetadata'):
