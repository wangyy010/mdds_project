...
DIFSECT = 0xFFFFFFFC;  #DIFFAT 分配表
FATSECT = 0xFFFFFFFD;   # FAT分配表
ENDOFCHAIN = 0xFFFFFFFE;  # 作为一个扇区链的结束
FREESECT = 0xFFFFFFFF;   # 空扇区
        ...
        if not is_ole_document and zipfile.is_zipfile(self.file):
            ...
            for name in zfile.namelist():
                if name.endswith(BINFILE_NAME):
                    ...
                    self.f.write(data)
                    self.f.seek(0)  # rewind the data file to the beginning seek() 方法用于移动文件读取指针到指定位置。
        else:
            self.f = open(self.file, 'rb')
...
class Header:
    def __init__(self, data, parser_options=None):
        ...
        self._abSig = self.header[0]    #  固定的0xD0 0xCF 0x11 0xE0 0xA1 0xB1 0x1A 0xE1
        self._clid = self.header[1]   # 从008H到017H的16字节，是Class Id，不过很多文件都置的0
        self._uMinorVersion = self.header[2]   # 从018H到019H的2字节UInt16，是文件格式的次要版本。
        self._uDllVersion = self.header[3]    #从01AH到01BH的2字节UInt16，是文件格式的主要版本。
        self._uByteOrder = self.header[4]   #01CH到01DH的2字节UInt16，是固定为0xFE 0xFF，表示文档使用的是Little Endian（低位在前，高位在后）
        self._uSectorShift = self.header[5]    # 从01EH到01FH的2字节UInt16，是Sector大小的幂，默认为9（0x09 0x00），即每个Sector为512字节。
        self._uMiniSectorShift = self.header[6]  #从020H到021H的2字节UInt16，是Mini-Sector大小的幂，默认为6（0x06 0x00），即每个Mini-Sector为64字节。
        self._usReserved = self.header[7]   # 从022H到023H的2字节UInt16，是预留的，必须置0。
        self._usReserved1 = self.header[8]  # 从024H到027H的4字节UInt32，是预留的，必须置0。
        self._usReserved2 = self.header[9]  # 从028H到02BH的4字节UInt32，是预留的，必须置0。
        self._csectFat = self.header[10] # number of sects in FAT chain 从02CH到02FH的4字节UInt32，是FAT的数量。
        self._sectDirStart = self.header[11] # first sect in Directory chain是Directory开始的SectorID。
        self._signature = self.header[12]  #从034H到037H的4字节UInt32，是用于事务的，必须置0。
        self._ulMiniSectorCutoff = self.header[13]  #从038H到03BH的4字节UInt32，是最小串（Stream）的最大大小，默认为4096（0x00 0x10 0x00 0x10）
        self._sectMiniFatStart = self.header[14] # first mini-FAT sect   #从03CH到03FH的4字节UInt32，是MiniFAT表开始的SectorID。
        self._csectMiniFat = self.header[15] # number of sects in mini-FAT  #从040H到043H的4字节UInt32，是MiniFAT表的数量。
        self._sectDifStart = self.header[16] # first sect in DIF chain  #从044H到047H的4字节UInt32，是DIFAT开始的SectorID。
        self._csectDif = self.header[17] # number of sets in DIF chain  #从048H到04BH的4字节UInt32，是DIFAT的数量。
        self._sectFat = self.header[18:] # sects of first 109 FAT sectors  #从04CH到1FFH的436字节UInt32[]，是前109块FAT表的SectorID。
...
class Directory:
    def __init__(self, data, index):
        ...
        self._ab = self.directory[0] #存储DirectoryEntry名称的，并且是以Unicode存储的，即每个字符占2个字节，其实可以看做是UInt16。
        self._cb = self.directory[1]  #是DirectoryEntry名称的长度（包括最后的“\0”）。
        # convert wide chars into ASCII
        self.name = ''.join([x for x in self._ab[0:self._cb] if ord(x) != 0])
        self._mse = self.directory[2] #是DirectoryEntry的类型。（主要的有：1为目录，2为节点，5为根节点）
        self._bflags = self.directory[3]  #
        self._sidLeftSib = self.directory[4] #是该DirectoryEntry左兄弟的EntryID（第一个DirectoryEntry的EntryID为0，下同）
        self._sidRightSib = self.directory[5] #是该DirectoryEntry右兄弟的EntryID。
        self._sidChild = self.directory[6] #是该DirectoryEntry一个孩子的EntryID。
        self._clsId = self.directory[7] #是该DirectoryEntry开始的SectorID。
        self._dwUserFlags = self.directory[8]
        self._time = [ self.directory[9], self.directory[10] ]
        self._sectStart = self.directory[11]
        self._ulSize = self.directory[12] #是该DirectoryEntry开始的SectorID。
...
if __name__ == '__main__':
    ...
    while options.extract_macros:
        ...
        while True:
            ...
            if check == 0x002F:
                # REFERENCECONTROL A REFERENCECONTROL (section 2.3.4.2.2.3) specifies a reference to external ActiveX controls that are used by the VBA project.
            ...
            if check == 0x000D:
                # REFERENCEREGISTERED A REFERENCEREGISTERED (section 2.3.4.2.2.5) specifies a reference to external Automation type libraries that are used by the VBA project.
            ...
            if check == 0x000E:
                # REFERENCEPROJECT A REFERENCEPROJECT (section 2.3.4.2.2.6) specifies a reference to external VBA projects that re used by the VBA project.
